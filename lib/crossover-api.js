...
//   let listRef = ref.child(endPoints.lists);
//   return listRef
//     .orderByChild('name')
//     .equalTo(listName)
//     .once('value')
...
//   return ref.child(endPoints.listSeries).child(key).remove()
//     .then(() => ref.child(endPoints.lists).child(key).remove());
// }
...
function populateTimelineByListName( listName ) {
  return getListByName(listName)
    .then((snapshot)=> populateTimeline(snapshot.key));
}

function populateTimeline( listId ) {
  console.log(`populateTimeline(${listId})`);

  ref.child(endPoints.listSeries).child(listId).once('value').then((data) => console.log(data.val()));
