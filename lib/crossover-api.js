...
function updateUserList(userId, listObj, listKey) {
  if (!userId) return Promise.reject('userId is null');

  const userListRef = ref.child(endPoints.userLists).child(userId);

  if (listKey)
    return userListRef.child(listKey).set(listObj).then(() => listKey);
  else
    return userListRef.push(listObj).then((snapshot) => snapshot.key);
}
  ...
  return sourceRef.once('value').then((snapshot) => {
    ...
    return loadSeriesEpisodes(list.series)
      .then((episodes) => {
        ...
        // return targetRef.set(list).then((resultSnapshot) => {
  ...
  return listRef.once('value').then((snapshot) => {
    ...
    return loadSeriesEpisodes(list.series)
      .then((episodes) => {
          ...
          const targetRef = ref.child(endPoints.userLists).child(userId).child(snapshot.key);
          const actions = episodes.map((obj) => saveToTimeline(obj, targetRef));
          // TODO: clear/remove list when already exists?
